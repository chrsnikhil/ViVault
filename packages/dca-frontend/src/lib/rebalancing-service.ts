...
export class RebalancingService {
  ...
  shouldExcludeToken(tokenAddress: string, symbol: string): boolean {
    ...
    if (symbol === 'USDC' || tokenAddress.toLowerCase() === this.usdcAddress.toLowerCase()) {
      return true;
    }

    // Skip ETH tokens - we don't want to swap ETH to USDC (use WETH instead)
    if (symbol === 'ETH' || tokenAddress === 'ETH') {
      return true;
    }

    // Skip zero balance tokens
    return false; // Let the main logic handle zero balance filtering
  }

  /** Test if a token has a valid route to USDC (for debugging) */
  async testTokenRoute(tokenIn: string, tokenOut: string, amountIn: string): Promise<boolean> {
    try {
      console.log(`🧪 Testing route: ${tokenIn} -> ${tokenOut} (${amountIn})`);
  ...
  calculateRebalancePlan(
    ...
    for (const token of vaultBalances) {
      ...
      console.log(
        `🔍 Address match: ${token.address.toLowerCase() === this.usdcAddress.toLowerCase()}`
      );
      ...
      if (currentBalanceWei.isZero()) {
        console.log(
          `⏭️ Skipping token with zero balance: ${token.symbol} (${token.address}) - Balance: ${token.balance}`
        );
        continue;
      }
      ...
      // Skip tokens with very small balances (less than 0.000001)
      if (currentBalanceFormatted < 0.000001) {
        console.log(
          `⏭️ Skipping token with very small balance: ${token.symbol} (${token.address}) - Balance: ${currentBalanceFormatted}`
        );
        continue;
      }
      ...
      console.log(
        `🔍 UI Balance: ${currentBalanceFormatted} (${currentBalanceWei.toString()} wei)`
      );
      ...
      const remainingBalanceFormatted = parseFloat(
        ethers.utils.formatUnits(remainingBalanceWei, token.decimals)
      );

      console.log(
        `🔍 Current Balance: ${currentBalanceFormatted} (${currentBalanceWei.toString()} wei)`
      );
      console.log(
        `🔍 Amount to Swap (${percentage}%): ${amountToSwapFormatted} (${amountToSwapWei.toString()} wei)`
      );
      console.log(
        `🔍 Remaining: ${remainingBalanceFormatted} (${remainingBalanceWei.toString()} wei)`
      );
      console.log(`🔍 DEBUG: amountToSwapWei.toString() = "${amountToSwapWei.toString()}"`);

      // Only include tokens with meaningful amounts to swap
      if (amountToSwapFormatted > 0.000001) {
        // Minimum 0.000001 tokens (lowered for debugging)
        const tokenPlan = {
        ...
        console.log(
          `✅ INCLUDING TOKEN: ${token.symbol} - Amount to swap: ${amountToSwapFormatted}`
        );
        ...
        console.log(
          `❌ EXCLUDING TOKEN: ${token.symbol} - Amount to swap too small: ${amountToSwapFormatted} (min: 0.000001)`
        );
      }
    }
    ...
    console.log('🔍 ===== FINAL REBALANCE PREVIEW =====');
    console.log('🔍 Total tokens:', preview.totalTokens);
    console.log('🔍 Total amount to swap:', preview.totalAmountToSwap);
    console.log('🔍 Estimated USDC received:', preview.estimatedUSDCReceived);
    console.log('🔍 Token plans count:', preview.tokenPlans.length);
    console.log('🔍 ===================================');
  ...
  async calculateRebalancePlanWithRoutes(
    ...
    for (const tokenPlan of basicPlan.tokenPlans) {
      console.log(
        `🔍 Checking route for ${tokenPlan.symbol} (${tokenPlan.tokenAddress}) -> USDC (${this.usdcAddress})...`
      );
  ...
  private async checkSwapRouteExists(
    tokenIn: string,
    tokenOut: string,
    amountIn: string
  ): Promise<boolean> {
    try {
      ...
      console.log(
        `🔍 Route check result: ${hasRoutes ? 'EXISTS' : 'NOT FOUND'} (${routes.length} routes)`
      );
      if (routes.length > 0) {
        console.log(
          `🔍 Available routes:`,
  ...
  async executeRebalancing(
    ...
    let totalUsdcReceivedWei = ethers.BigNumber.from(0);
    ...
    try {
      ...
      console.log(
        `🔍 Found valid routes for ${validTokenPlans.length} out of ${plan.tokenPlans.length} tokens`
      );
      ...
      for (const tokenPlan of validTokenPlans) {
        try {
          ...
          console.log(
            ...
            ethers.utils.formatUnits(amountInWei, tokenPlan.decimals)
          );
          ...
          if (!expectedAmount.eq(amountInWei)) {
            throw new Error(
              `Amount mismatch! Expected: ${expectedAmount.toString()}, Got: ${amountInWei.toString()}`
            );
          }
          ...
          console.log(
            `🔍 DEBUG: Swap amount - wei: ${tokenPlan.amountToSwap}, formatted: ${formattedAmount}`
          );
          ...
          if (pkpBalance.gt(0)) {
            ...
            console.log(
              '🔍 USDC received for this swap:',
              ethers.utils.formatUnits(pkpBalance, 6),
              'USDC'
            );

            // Track total USDC received
            totalUsdcReceivedWei = totalUsdcReceivedWei.add(pkpBalance);
  ...
  private async transferUsdcToVault(
    ...
    try {
      ...
      console.log(
        '⚠️ USDC transfer to vault not implemented yet - would transfer:',
        ethers.utils.formatUnits(pkpBalance, 6),
        'USDC'
      );
